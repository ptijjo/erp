...
export default function TresoreriePage() {
  ...
  const closeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      alert(
        "Période clôturée. Les ventes, sorties budgétaires et écritures journal rétroactives sont bloquées.",
      );
    },
    onError: (e) => {
  ...
  const reopenMutation = useMutation({
    mutationFn: async (payload: {
      ...
      await api.post("/treasury/accounting-periods/reopen", {
        year: payload.year,
        month: payload.month,
    ...
    onSuccess: () => {
      ...
      alert("Période rouverte.");
    },
    onError: (e) => {
      alert(apiErrorMessage(e, "Réouverture impossible"));
    },
  });
                        ...
                        {canManage ? (
                          <Button
                            ...
                            onClick={() => {
                              if (
                                window.confirm(
                                  `Rouvrir ${MONTHS_FR[c.month - 1]} ${c.year} (${c.organization?.name ?? "Groupe"}) ?`,
                                )
                              ) {
